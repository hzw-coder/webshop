<template>
  <div id="container" v-if="goodsDetail[0]">
    <div class="pro_detail">
      <div class="pro_img">
        <div class="tb_booth">
          <img class="pro_middle_img" :src="goodsDetail[0].image_url" />
        </div>
      </div>
      <div class="pro_meg">
        ...
        <div class="pro_meg_console">
          ...
          <div class="shopping_car">
            <el-button
              type="danger"
              @click="dealWithCellBtnClick(goodsDetail[0])"
              >加入购物车</el-button
            >
          </div>
        </div>
    ...
    <div class="pro_judge" v-if="userInfo.id">
      <h3>评价该商品</h3>
      <span>为该商品评分</span>
      <el-rate
        v-model="rating"
    ...
    <div class="pro_judge" v-else>
      <h3>评价该商品</h3>
      <span class="judge_error_tip">登录后才可发表评论</span>
    </div>
  </div>
</template>
<script>
...
import { postComment, addGoodsToCar } from "../../api/index";
export default {
  name: "goods",
  data() {
    return {
  ...
  computed: {
    ...mapState(["goodsDetail", "goodsComment", "userInfo"]),
  },
  created() {
