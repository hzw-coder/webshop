<template>
  <div v-if="categoryList.length>0">
    ...
    <div id="footer">
      <ul class="pagination">
        <li><a class="pag_back" @click.prevent="getMore(activeIndex-1)">«</a></li>
        <li>
        <!-- v-for迭代数字遍历出所有的页码 -->
          <a
            class="pag_a"
            :class="{ pag_active: activeIndex === item }"
            :key="index"
            v-for="(item,index) in catePages[this.currentCate - 1]"
            @click.prevent="getMore(item)"
            >{{ item }}</a
          >
        </li>
        <li>
          <a class="pag_go" @click.prevent="getMore(activeIndex+1)">»</a>
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...mapState(["categoryList",'recommendshoplist']),
    catePages(){
        let arr = []
        this.categoryList.forEach((item,index) => {
            //页数(总数/每页数量)
            let page = Math.ceil(item.cate_counts / this.pageSize) //向上取整
            arr.push(page)
        })
        // console.log(arr);
        return arr
  ...
  methods: {
    ...
    handleCommand(command) {
        ...
        this.$router.push('/search/' + command + '/1')
    ...
    getMore(index) {
        ...
        this.$router.push('/search/'+this.currentCate+'/'+this.activeIndex)
        }
    },
  },
  components: {
